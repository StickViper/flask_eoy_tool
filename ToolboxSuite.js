...
// Protected uppercase strings (credentials, degrees, suffixes)
const PROTECTED_CREDENTIALS = [
...
function fixCapitalization(name) {
  ...
  // Step 1: Fix credential periods (uppercase AND lowercase from ProperCase damage)
  // Uppercase: M.D. → MD, F.N.P. → FNP, A.R.N.P. → ARNP
  name = name.replace(/\b([A-Z])\.([A-Z])\.([A-Z])\.([A-Z])\b/g, '$1$2$3$4');
  name = name.replace(/\b([A-Z])\.([A-Z])\.([A-Z])\b/g, '$1$2$3');
  name = name.replace(/\b([A-Z])\.([A-Z])\b/g, '$1$2');

  // Lowercase: m.d. → MD, f.n.p. → FNP (from ProperCase damage)
  name = name.replace(/\b([a-z])\.([a-z])\.([a-z])\.([a-z])\b/g, (m, a, b, c, d) =>
    a.toUpperCase() + b.toUpperCase() + c.toUpperCase() + d.toUpperCase());
  name = name.replace(/\b([a-z])\.([a-z])\.([a-z])\b/g, (m, a, b, c) =>
    a.toUpperCase() + b.toUpperCase() + c.toUpperCase());
  name = name.replace(/\b([a-z])\.([a-z])\b/g, (m, a, b) =>
  ...
  for (let word of words) {
    ...
    // Force credentials to uppercase
    if (PROTECTED_CREDENTIALS.includes(cleanWord.toUpperCase())) {
      fixed.push(cleanWord.toUpperCase() + punctuation);
    }
