...
- Organizes 737 providers into 11 review categories

**Status:** Read-only mode functional, write phase in development
...
flask_eoy_tool/
├── scripts/
│   ├── eoy_tool.py              # Main Flask application (1349 lines)
...
- **December 2024 cleanup** in progress using Flask EOY tool
...
### Synagogue List
- Future campaign (Canavan Foundation outreach)
- Same tools/infrastructure will be adapted
...
### EOY Cleanup Tool
- **Fuzzy matching** to validate orders (70% name, 30% address weighting)
- **Duplicate detection** - exact matches, networks (same phone), similar providers
- **Status validation** - voicemail callbacks, email follow-ups, invalid providers
- **11 review categories** for systematic cleanup
- **API efficiency** - 11 calls vs 737 (98.5% reduction), well under 60/min quota

### ToolboxSuite.js (Apps Script)
- Note merging with semicolon separation
- Dropdown status updates with automatic coloring
- Stats snapshot export to TSV
- Debug/repair sidebar for data issues
...
2. Check `TODO.md` section 0 for current priorities
3. Review recent git commits: `git log --oneline -10`

**When making changes:**
- Update documentation in same commit as code changes
- Add "Last Updated: YYYY-MM-DD" to modified docs
- Test on both OBGYN and PCP sheets if changing ToolboxSuite.js
...
For questions about this codebase, see `AGENT_PRINCIPLES.md` and `TODO.md` for current context.
